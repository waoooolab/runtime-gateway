...
def _resolve_worker_error_status(exc: RuntimeExecutionClientError) -> int:
    return resolve_upstream_status_code(
...
def dispatch_worker_status(
    *,
    ...
    try:
        validate_runtime_worker_status_contract(result)
    except ContractValidationError as exc:
        emit_audit_event(
            action=action,
            decision="deny",
            ...
            metadata={
                "reason": str(exc),
                "validation_schema": "runtime/runtime-worker-status.v1.json",
            },
        )
        raise HTTPException(status_code=502, detail=f"invalid worker status response: {exc}") from exc
